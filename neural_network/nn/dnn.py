...
class Mnist(object):
    ...
    def forward(self, train_data):
        self.neurons["z2"] = fc_forward(train_data, self.weights["W1"], self.weights["b1"])
        self.neurons["z2_relu"] = relu_forward(self.neurons["z2"])
        self.neurons["z3"] = fc_forward(self.neurons["z2_relu"], self.weights["W2"], self.weights["b2"])
        self.neurons["z3_relu"] = relu_forward(self.neurons["z3"])
        self.neurons["y"] = fc_forward(self.neurons["z3_relu"], self.weights["W3"], self.weights["b3"])
        return self.neurons["y"]

    def backward(self, train_data, y_true):
        loss, self.gradients["y"] = cross_entropy_loss(self.neurons["y"], y_true)
        self.gradients["W3"], self.gradients["b3"], self.gradients["z3_relu"] = fc_backward(self.gradients["y"],
                                                                                            self.weights["W3"],
                                                                                            self.neurons["z3_relu"])
        self.gradients["z3"] = relu_backward(self.gradients["z3_relu"], self.neurons["z3"])
        self.gradients["W2"], self.gradients["b2"], self.gradients["z2_relu"] = fc_backward(self.gradients["z3"],
                                                                                            self.weights["W2"],
                                                                                            self.neurons["z2_relu"])
        self.gradients["z2"] = relu_backward(self.gradients["z2_relu"], self.neurons["z2"])
